...
fs.writeFileSync(outputFile, 'Estrutura de pastas e arquivos com conteúdo:\r\n', { encoding: 'utf8' });
        ...
        if (item.isDirectory()) {
            ...
            if (includeContent) {
                try {
                    ...
                    console.error(`Erro ao ler o conteúdo do arquivo '${relativePath}': ${error.message}`);
                    fs.appendFileSync(outputFile, `\`\`\`plaintext\r\n`, { encoding: 'utf8' });
                    fs.appendFileSync(outputFile, `Erro ao ler o conteúdo do arquivo '${relativePath}': ${error.message}\r\n`, { encoding: 'utf8' });
                    fs.appendFileSync(outputFile, '```\r\n', { encoding: 'utf8' });
                }
            }
        }
